...
int main() {
    char username[255];
    char password[255];
    char salt[SALT_LEN + 1];  // 盐数组，预留空间以存储字符串结尾符 '\0'
    unsigned char hashed_password[MD5_DIGEST_LENGTH];  // MD5 哈希（16 字节）
    ...
    MD5_Final(hashed_password, &md5_ctx);  // 获取最终的哈希值

    // 打印 MD5 哈希（加密后的密码）
    printf("加密后的密码: ");
    print_binary(hashed_password, MD5_DIGEST_LENGTH);  // 打印二进制哈希值
